...
1. **Deduplicación por Identificador Exacto**: `(fuente, tipo_operacion, id_anuncio)` $\rightarrow$ Descarta publicaciones idénticas repetidas por paginación.
2. **Deduplicación por URL Canónica**: `(tipo_operacion, url)` $\rightarrow$ Elimina endpoints web duplicados.
3. **Deduplicación por Huella Física del Inmueble**: `(tipo_operacion, titulo, precio_soles, area_total, distrito)` $\rightarrow$ Filtra inmuebles sindicados simultáneamente por diferentes agentes o agencias.
...
* **Saneamiento de Textos y Entidades HTML**: Se decodificaron entidades HTML (`&amp;`, `&quot;`, `m&sup2;`) y se eliminaron saltos de línea (`\r\n\t`) y caracteres corruptos (*mojibake*) para asegurar un formato CSV uniforme.
